...
self.onmessage = (e) => {
  img1 = new Uint8ClampedArray(e.data.img1);
  img2 = new Uint8ClampedArray(e.data.img2);
  diff = new Uint8ClampedArray(img1);
  width = e.data.width;
  height = e.data.height;
  init = e.data.init;
  key = e.data.key;
  try {
    pixelmatch(img1, img2, diff, width, height, init);
    self.postMessage({
      diff: diff.buffer,
      key: key
    }, [diff.buffer]);
